...
install_cloudflare_packages() {
    ...
    export DEBIAN_FRONTEND=noninteractive; . /etc/os-release
...
register_account() {
    mkdir -p /etc/warp; cd /etc/warp || exit
...
build_config() {
    cd /etc/warp || exit; wgcf generate >> "${WARP_LOG}" 2>&1
...
execute_warp_install() {
    local target=$1; echo ""
    install_warp_prereqs; install_wgcf
    if ! install_cloudflare_packages; then echo -e "\n[ ${RED}✖${NC} ] Critical failure. Aborting."; sleep 3; return; fi
    register_account
    ...
    else echo -e "\n[ ${RED}✖${NC} ] Failed to start WireGuard."; fi
    sleep 3
}

toggle_warp_service() {
    echo ""
    if [ ! -f "/etc/wireguard/wgcf.conf" ]; then echo -e "[ ${RED}✖${NC} ] WARP is not installed."; sleep 2; return; fi
...
uninstall_warp() {
    ...
    else echo -e "[ ${RED}✖${NC} ] WARP is not installed."; fi
    pause_execution
}
...
draw_warp_dashboard() {
    ...
    if [[ ${WARP_IPv4_Status} == "on" || ${WARP_IPv4_Status} == "plus" ]]; then
        v4_vps_out="${YELLOW}${VPS_IPv4_Int}${NC}"; v4_warp_out="${GREEN}${WARP_IPv4_IP}${NC}${active_tag}"
    else
        v4_vps_out="${GREEN}${VPS_IPv4_Int}${NC}${active_tag}"; v4_warp_out="${RED}------------${NC}"
    fi

    if [[ ${WARP_IPv6_Status} == "on" || ${WARP_IPv6_Status} == "plus" ]]; then
        v6_vps_out="${YELLOW}${VPS_IPv6_Int}${NC}"; v6_warp_out="${GREEN}${WARP_IPv6_IP}${NC}${active_tag}"
    else
        v6_vps_out="${GREEN}${VPS_IPv6_Int}${NC}${active_tag}"; v6_warp_out="${RED}------------${NC}"
    fi

    echo -e "${BOLD_BLUE}-----------------------------------------------------${NC}"
    echo -e "${BOLD_BLUE}                   Cloudflare WARP                   ${NC}"
    echo -e "${BOLD_BLUE}-----------------------------------------------------${NC}"
    echo -e " VPS  (IPv4) : ${v4_vps_out}\n WARP (IPv4) : ${v4_warp_out}\n\n VPS  (IPv6) : ${v6_vps_out}\n WARP (IPv6) : ${v6_warp_out}"
    echo -e "${BOLD_BLUE}-----------------------------------------------------${NC}"
}

manage_warp() {
    while true; do
        clear; draw_warp_dashboard
        echo ""; echo "1. Install WARP (Free)"; echo "2. Install WARP+ (Key)"; echo "3. Toggle WARP On/Off"; echo "4. Uninstall WARP"; echo "5. View WARP Logs"; echo "0. Return"; echo ""
        read -rp "Select option: " choice
        case "$choice" in
            1) echo -e "\nTarget: \n1- IPv4 \n2- IPv6\n3- IPv4 & IPv6 (Both)\n"; read -rp "Select option: " t
               if [[ "$t" =~ ^[1-3]$ ]]; then execute_warp_install "$t"; fi ;;
            2) echo ""; read -rp "Enter WARP+ Key: " k
               if [ -n "$k" ]; then
                   echo -e "\nTarget: \n1- IPv4 \n2- IPv6\n3- IPv4 & IPv6 (Both)\n"; read -rp "Select option: " t
                   if [[ "$t" =~ ^[1-3]$ ]]; then
                       install_warp_prereqs; install_wgcf
                       if install_cloudflare_packages; then
                           register_account; sed -i "s/\(license_key = \).*/\1'${k}'/" "/etc/warp/wgcf-account.toml"
            ...
            4) uninstall_warp ;;
            5) clear; echo -e "${BOLD_BLUE}-----------------------------------------------------${NC}\n${BOLD_BLUE}                   WARP Debug Logs                   ${NC}\n${BOLD_BLUE}-----------------------------------------------------${NC}"
               echo -e "Streaming last 50 lines. Press Ctrl+C to exit.\n"; trap 'true' SIGINT; tail -n 50 -f "${WARP_LOG}"; trap cleanup SIGINT SIGTERM ;;
            0) break ;;
