...
core_setup() {
    ...
    sleep 1
    if ! command -v openvpn >/dev/null 2>&1; then apt-get install -y openvpn openssl iptables iptables-persistent iproute2 > /dev/null 2>&1; fi
...
EOCONF
    ...
    if command -v ufw >/dev/null 2>&1; then sed -i 's/DEFAULT_FORWARD_POLICY="DROP"/DEFAULT_FORWARD_POLICY="ACCEPT"/' /etc/default/ufw; ufw reload >/dev/null 2>&1; fi
